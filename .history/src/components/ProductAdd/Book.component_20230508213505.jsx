...
function BookComponent(props) {
  ...
  const handleWeightChange = (event) => {
    const value
    const regex = /^[0-9\b]+$/;
    if (event.target.value === "" || regex.test(event.target.value)) {value = event.target.value;
      setWeight(value);
    }
  ...
  return (
    <>
      ...
      <Form.Group className="mb-3 mt-3" id="Book">
        <Form.Label>Weight (KG)</Form.Label>
        <Form.Control type="string" placeholder="#weight" value={weight} onChange={handleWeightChange}/>
